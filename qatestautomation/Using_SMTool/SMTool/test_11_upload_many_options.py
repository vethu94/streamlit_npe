...
for active_com in init.active_com_list:
    ...
    if int(ai.control_command(init.tool_title, init.control_file_box["obst"], "IsEnabled")):
        ...
        time.sleep(1)
       #voice
    if int(ai.control_command(init.tool_title, init.control_file_box["voice"], "IsEnabled")):
        print("Option already checked!")
    else:
        ai.control_click(init.tool_title, init.control_check_box["voice"])
        time.sleep(1)
    ...
    time.sleep(1)
    #enter path to voice file 
    ai.control_set_text(init.tool_title, init.control_file_box["voice"], init.voice_path)
    time.sleep(1)
    ...
    if repo.wait_while_not_in_text("Successfull installation", init.popup_title, init.timeout_vlong):
